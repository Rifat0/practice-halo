...
const changeAvatar = async (formData) => {
  const session = await auth();
  const response = await fetch(`${process.env.BACK_END_URL}/api/update-user-avatar`, {
    method: 'POST',
    body: formData,
    headers: {
      'Accept': 'application/json',
...
export {
  changeAvatar, getAppointmentNotes, getAppointments, getGeneralNotes, getInvoices, getUserAnalytics, login,
