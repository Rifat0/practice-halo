(function ($) {
  ...
  if ($("#pageWrapper").hasClass("compact-wrapper")) {
    ...
    jQuery(".sidebar-title").click(function () {
      ...
      if (jQuery(this).next().is(":hidden") == true) {
        ...
        jQuery(this)
          .find("div")
          .replaceWith(
            '<div className="according-menu"><i className="fa fa-angle-down"></i></div>'
          );
    ...
    var contentwidth = jQuery(window).width();
    if (contentwidth > 992) {
      $('<div className="bg-overlay1"></div>').appendTo($("body"));
    }
  ...
  $(window).on("overlay", function () {
    ...
    if ($(window).width() <= 1184 && !$isHidden && $bgOverlay.length === 0) {
      $('<div className="bg-overlay active"></div>').appendTo($("body"));
    }
